...
func (server *Server) signUp() {
	...
	*/
	...
	// 写入本地文件
	server.UserFile.WriteString("\n" + lyr.NumToString(newUser.UID) + " " + newUser.UserName + " " + userNameAndPassword[1])
	...
	if len(splitText) == 2 {
		...
		if err == nil {
			if tarUserContent, ok := server.AllUsers[tarUID]; ok {
				...
				server.GroupFile.WriteString("\n" + server.LastGID + " " + newGroup.GroupName + " " + (lyr.NumToString(jsonText.UID) + "|" + lyr.NumToString(tarUID)))
...
func (server *Server) addGroup() {
	...
	if _, ok := server.AllGroups[tarGID]; ok {
		...
		// 写入本地文件
		server.GroupFile.WriteString("\nAPPEND " + tarGID + " " + lyr.NumToString(jsonText.UID))
